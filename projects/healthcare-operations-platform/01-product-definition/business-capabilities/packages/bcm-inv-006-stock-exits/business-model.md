...
- id: ENT-SXT-001
  ...
  description: Append-only exit record; each row is one non-consumption, non-waste
    stock decrement.
  fields:
  - name: stockExitId
    type: uuid
...
- id: INV-SXT-002
  statement: An exit with exitReason branch_transfer or internal_transfer must supply
    destinationBranchId.
- id: INV-SXT-003
  statement: Confirming a StockExitRecord is the only trigger allowed to invoke ApplyStockExit
