---
id: HOP-PROC-BCM-AI-006
format: markdown_structured_payload
type: process-model
name: Safety Policy and Human Review Processes
version: 1.0.0
status: modeled
---

# Safety Policy and Human Review Processes

<!-- NEXORA_STRUCTURED_PAYLOAD_V1 -->

## Structured Payload

```yaml
artifact:
  id: HOP-PROC-BCM-AI-006
  type: process-model
  status: modeled
capability_id: BCM-AI-006
processes:
  - id: PROC-AI-011
    name: Safety Policy and Human Review process 1
    steps:
    - capture tenant, actor, purpose and allowed source context
    - enforce AI safety policy and provider routing policy
    - prepare draft output with citations and confidence metadata
    - route to human review when policy or confidence requires it
    - persist audit evidence
  - id: PROC-AI-012
    name: Safety Policy and Human Review process 2
    steps:
    - capture tenant, actor, purpose and allowed source context
    - enforce AI safety policy and provider routing policy
    - prepare draft output with citations and confidence metadata
    - route to human review when policy or confidence requires it
    - persist audit evidence
```
