---
id: HOP-TRACE-BCM-SVC-002
format: markdown_structured_payload
type: traceability
name: Test Catalog Traceability
version: 0.2.0
status: modeled
---
...
traces:
  ...
  - process: PRC-SVC-002-01
    commands:
    - CreateTestDefinition
  - process: PRC-SVC-002-02
    commands:
    - PublishTestDefinition
  - process: PRC-SVC-002-03
    commands:
    - DeprecateTestDefinition
  api_to_permissions:
  - operation: createTest
    scope: catalog.test.write
  - operation: publishTest
    scope: catalog.test.publish
  events_to_audit:
  - event: TestDefinitionPublished
    audit_sink: BCM-PLT-007
  ui_to_api:
  - screen: SCR-SVC-002-02
    operations:
    - createTest
    - updateTest
  ...
  - roadmap_group: COM-MOD-011
    ...
    note: Public website discovery surface modeled by extending openapi-source.md,
      ...
      package, aggregate or schema was created; reuses TestDefinition/AGG-006, PublishedTestSnapshot
      and getPublishedTestSnapshot.
  - roadmap_group: COM-MOD-011
    backlog_item: COM-MOD-011-BE-001
    surface_added: public_website (runtime discovery)
    note: Compiled GET /api/public/catalog/tests/published and /api/public/catalog/tests/{testId}/published-snapshot
      via the new catalogtestconfiguration::catalog-public-read-port named interface.
      TestCatalogService.listPublished added; endpoints only return status=published
      records.
  - roadmap_group: COM-MOD-011
    backlog_item: COM-MOD-011-WEB-001
    surface_added: public_website (frontend discovery)
    note: Compiled TestsPage (list) and TestDetailPage (snapshot, including methodology/measurement
      unit/result type/turnaround time) at 07-implementation/public-website/src/pages/,
      consuming GET /api/public/catalog/tests/published and /api/public/catalog/tests/{testId}/published-snapshot.
      Tests and panels are also combined into a single picker (useCatalogItemOptions)
      for the appointment/quotation request forms.
  - roadmap_group: COM-MOD-011
    ...
    note: Compiled PublicContentReviewScreen at 07-implementation/employee-portal/src/components/screens/,
      consuming GET /api/public/catalog/tests/published (src/api/publicContentApi.ts)
  ...
  - roadmap_group: COM-MOD-011
    ...
    note: Validated public test catalog surface end to end with zero vulnerabilities
  ...
  - roadmap_group: COM-MOD-011
    ...
    note: Formally closed COM-MOD-011 Public Website and Digital Growth module. Confirmed
      reused public test catalog surfaces, zero vulnerability findings, zero coverage
