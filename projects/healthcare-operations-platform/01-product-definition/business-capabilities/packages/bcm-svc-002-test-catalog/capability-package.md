---
id: HOP-CAP-PKG-BCM-SVC-002
format: markdown_structured_payload
type: capability-package
name: Test Catalog Capability Package
version: 0.2.0
status: modeled
---

# Test Catalog Capability Package
...
scope:
  summary: 'Defines individual diagnostic tests including method, measurement unit,
    result type, turnaround time and references to analytes, samples and preparations.
    The test is the atomic orderable clinical definition.

    '
  in_scope:
  - Test definition, methodology and result typing.
  - Test lifecycle draft, published, deprecated.
  - References to analytes, sample requirements and preparation instructions.
  - Published test snapshot exposure for orders and processing.
  out_of_scope:
  - Panel composition (BCM-SVC-003).
...
product_surfaces:
  ...
  public_website_note: Realized by COM-MOD-011-DEF from the pre-existing openapi-source.md
    future_surfaces placeholder. Reuses the existing getPublishedTestSnapshot operation
    and PublishedTestSnapshot schema; no new aggregate, schema or capability package
    created.
required_artifacts:
