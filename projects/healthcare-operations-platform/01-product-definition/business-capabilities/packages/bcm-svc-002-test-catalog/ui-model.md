---
id: HOP-UI-BCM-SVC-002
format: markdown_structured_payload
type: ui-model
name: Test Catalog UI Model
version: 0.2.0
status: modeled
---

# Test Catalog Ui Model

<!-- NEXORA_STRUCTURED_PAYLOAD_V1 -->

## Structured Payload

```yaml
artifact:
  id: HOP-UI-BCM-SVC-002
  type: ui-model
  name: Test Catalog UI Model
  version: 0.2.0
  status: modeled
  classification: editable_model
  capability: BCM-SVC-002
  target_surface: employee_portal
surfaces:
  employee_portal:
    status: required
    generatable: true
  patient_portal:
    status: read_only_later
    generatable: deferred
  doctor_portal:
    status: read_only_later
    generatable: deferred
  public_website:
    status: required
    generatable: not_applicable
    note: Public website pages are a COM-MOD-011-WEB-001 delivery concern, not an
      employee-portal screen. This capability only exposes the published-catalog read
      API (openapi-source.md public_surface) consumed by those public pages.
screens:
- id: SCR-SVC-002-01
  name: Test List
  route: /catalog/tests
  type: list
  scopes:
  - catalog.test.read
  components:
  - DataTable
  - StatusFilter
  - SearchBar
  generatable: true
- id: SCR-SVC-002-02
  name: Test Editor
  route: /catalog/tests/{testId}
  type: form
  scopes:
  - catalog.test.write
  components:
  - TestDetailForm
  - AnalyteLinkEditor
  - SampleRequirementLinkEditor
  - ResultTypeSelector
  generatable: true
- id: SCR-SVC-002-03
  name: Publish Test Dialog
  route: /catalog/tests/{testId}/publish
  type: confirmation
  scopes:
  - catalog.test.publish
  components:
  - PublicationChecklist
  - LinkedComponentEligibilitySummary
  generatable: false
  custom_reason: Displays cross-aggregate publication validation results.
states:
- draft
- published
- deprecated
- retired
localization:
  languages:
  - en
  - es
  default: es
```
