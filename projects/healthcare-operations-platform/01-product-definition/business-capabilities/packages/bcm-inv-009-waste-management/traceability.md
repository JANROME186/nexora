---
id: HOP-TRACE-BCM-INV-009
format: markdown_structured_payload
type: traceability
name: Waste Management Traceability
version: 0.1.0
status: modeled
---
...
traces:
  ...
  domain_foundation:
    ...
    aggregate_reference: AGG-013 InventoryItem (owned by BCM-INV-001; stockSummary
      disposal decrease delegated to this capability via ApplyWasteDisposal)
    ...
    note: BCM-QLT-007 Audit Management is listed as this capability's downstream_dependencies
      ...
      (roadmap MVP3); the WasteRecorded event is published in anticipation of that
      future consumer without requiring it to exist yet.
  brm_alignment:
  - rule: BRM-001-R018
    alignment: Audit records are append-only; every disposal is audited with a mandatory
      reason code (RN-002, RN-005).
  hrp_alignment:
  ...
  - process: PRC-WST-009-01
    commands:
    - ApplyWasteDisposal
  api_to_permissions:
  - operation: applyWasteDisposal
    scope: inventory.waste.manage
  - operation: listWasteRecords
    scope: inventory.waste.read
  events_to_audit:
  - event: WasteRecorded
    audit_sink: BCM-PLT-007
  ui_to_api:
  - screen: SCR-WST-009-01
    operations:
    - applyWasteDisposal
    - listWasteRecords
  ...
  compilation_evidence:
    backend_implementation: ../../../../../07-implementation/backend/src/main/java/com/nexora/hop/platformfoundation/inventoryquality/wastemanagement/
    ...
    notes: applyWasteDisposal and listWasteRecords compiled. Real-time lot remaining-quantity
      guard (RN-001) and mandatory reason code (RN-002) enforced. When disposal drives
      a lot's remainingQuantity to zero, the lot transitions to STATUS_DISPOSED (conditional
      cross-entity status transition, RN-005).
  backlog_items:
