---
id: HOP-TRACE-BCM-INV-007
format: markdown_structured_payload
type: traceability
name: Consumption Tracking Traceability
version: 0.1.0
status: modeled
---

# Consumption Tracking Traceability
...
traces:
  ...
  domain_foundation:
    ...
    aggregate_reference: AGG-013 InventoryItem (owned by BCM-INV-001; stockSummary
      decrease delegated to this capability via ApplyConsumption)
  ...
  - process: PRC-CNS-007-01
    commands:
    - ApplyConsumption
  api_to_permissions:
  - operation: applyConsumption
    scope: inventory.consumption.manage
  - operation: listConsumptionRecords
    scope: inventory.consumption.read
  events_to_audit:
  - event: ConsumptionRecorded
    audit_sink: BCM-PLT-007
  ui_to_api:
  - screen: SCR-CNS-007-01
    operations:
    - listConsumptionRecords
  consumed_by_capabilities:
  - capability: BCM-QLT-001
    relationship: Reads the consumed control-material lot for internal quality control
      run evidence.
  ...
  compilation_evidence:
    backend_implementation: ../../../../../07-implementation/backend/src/main/java/com/nexora/hop/platformfoundation/inventoryquality/consumptiontracking/
    ...
    notes: applyConsumption and listConsumptionRecords compiled. Reagent-typed items
      must have a BCM-INV-002 reagent profile before consumption (RN-001). Real-time
      lot-eligibility guard (RN-004) and INV-CAT-002 non-negative-onhand invariant
      are enforced before mutating InventoryItem.stockSummary and StockLot.remainingQuantity.
  backlog_items:
