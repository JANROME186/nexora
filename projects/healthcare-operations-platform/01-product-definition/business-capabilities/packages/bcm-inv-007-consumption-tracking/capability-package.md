---
id: HOP-CAP-PKG-BCM-INV-007
format: markdown_structured_payload
type: capability-package
name: Consumption Tracking Capability Package
version: 0.1.0
status: modeled
---

# Consumption Tracking Capability Package
...
scope:
  summary: 'Automatically decrements InventoryItem.stockSummary and the selected StockLot.remainingQuantity
    when a linked diagnostic test is performed, using BCM-INV-002''s reagentProfile.consumptionUnitRatio.
    Reads AGG-009 LaboratoryResult (owned by laboratory-results) read-only to determine
    which test was performed; never mutates LaboratoryResult. Feeds BCM-QLT-001 Internal
    Quality Controls with the control-material lot used for a QC run.

    '
  in_scope:
  - 'Consumption: decrease StockLot.remainingQuantity and InventoryItem.stockSummary.onHandQuantity
    when a linked test is performed.'
  - Read-only reference to LaboratoryResult/order test-performed events.
  out_of_scope:
  - LaboratoryResult mutation (owned exclusively by laboratory-results/BCM-LAB-006).
  - Reagent classification itself (BCM-INV-002).
  - Non-consumption exits and waste (BCM-INV-006/009).
roadmap:
  module: COM-MOD-010
  release: REL-002
