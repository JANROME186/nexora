---
id: HOP-TRACE-BCM-INV-008
format: markdown_structured_payload
type: traceability
name: Inventory Adjustments Traceability
version: 0.1.0
status: modeled
---
...
traces:
  ...
  domain_foundation:
    ...
    aggregate_reference: AGG-013 InventoryItem (owned by BCM-INV-001; stockSummary
      correction delegated to this capability via ApplyAdjustment)
  ...
  brm_alignment:
  - rule: BRM-001-R018
    alignment: Audit records are append-only; every adjustment is audited with dual-actor
      approval (RN-002, RN-005).
  hrp_alignment:
  ...
  - process: PRC-ADJ-008-01
    commands:
    - ApplyAdjustment
  api_to_permissions:
  - operation: applyAdjustment
    scope: inventory.adjustment.manage
  - operation: listAdjustments
    scope: inventory.adjustment.read
  events_to_audit:
  - event: StockAdjusted
    audit_sink: BCM-PLT-007
  ui_to_api:
  - screen: SCR-ADJ-008-01
    operations:
    - applyAdjustment
    - listAdjustments
  ...
  compilation_evidence:
    backend_implementation: ../../../../../07-implementation/backend/src/main/java/com/nexora/hop/platformfoundation/inventoryquality/inventoryadjustments/
    ...
    notes: applyAdjustment and listAdjustments compiled. Dual-actor rule (approverId
      != requestedBy, RN-002), mandatory reason code (RN-003), non-zero delta (RN-001)
      and INV-CAT-002 non-negative-onhand invariant all enforced before mutating InventoryItem.stockSummary
      and the referenced StockLot.remainingQuantity. Delta may be positive or negative.
  backlog_items:
