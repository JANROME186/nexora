---
id: HOP-CAP-PKG-BCM-QLT-003
format: markdown_structured_payload
type: capability-package
name: Calibration Management Capability Package
version: 0.1.0
status: modeled
---

# Calibration Management Capability Package
...
capability:
  ...
  primary_aggregate: InventoryItem (AGG-013, owned by BCM-INV-001; calibrationRecord
    delegated to this capability)
  process_ref: not_yet_defined_in_HRP-001
scope:
  summary: 'Records calibration events for equipment-classified InventoryItem records:
    standard used, result (pass/fail/adjusted), next due date and certificate reference.
    Holds delegated, field-scoped mutation authority over InventoryItem.calibrationRecord
    only. On a failed calibration, publishes CalibrationFailed for BCM-QLT-004 Equipment
    Management to react to (transition availabilityStatus to out_of_service) rather
    than writing equipmentProfile itself.

    '
  in_scope:
  - 'Calibration event recording: standard, result, nextDueDate, certificateReference.'
