---
id: HOP-TRACE-BCM-RES-002
format: markdown_structured_payload
type: traceability
name: PDF Report Generation Traceability
version: 0.1.0
status: modeled
---
...
traces:
  ...
  domain_foundation:
    ...
    aggregate_reference: GeneratedResultReport (new entity owned by this capability);
  ...
  brm_alignment:
  - rule: BRM-001-R012
    alignment: A report can only be generated from a medically-validated, released
      result (RN-001).
  - rule: BRM-001-R018
    alignment: Report generation and supersession events are audited (RN-006, RN-007).
  hrp_alignment:
  - process: HRP-001-P07 Result Report and Digital Delivery
    capability_role: Generate PDF report and store report document segment of the
      process.
  rules_to_tests:
  - rule: RN-001
    tests:
  ...
  - process: PRC-RPT-002-01
    commands:
    - GenerateResultReport
  - process: PRC-RPT-002-02
    commands:
    - RegenerateResultReport
  api_to_permissions:
  - operation: generateResultReport
    scope: report.generate
  - operation: getResultReport
    scope: report.read
  - operation: listResultReports
    scope: report.read
  events_to_audit:
  ...
  - event: ReportSuperseded
    audit_sink: BCM-PLT-007
  ui_to_api:
  - screen: SCR-RPT-002-01
    operations:
    - listResultReports
  - screen: SCR-RPT-002-02
    operations:
    - generateResultReport
  consumed_by_capabilities:
  - capability: BCM-RES-004
    relationship: Digital delivery reads GeneratedResultReport to attach the report
      artifact to a delivery.
