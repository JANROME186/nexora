...
import { getPatientHistory, getPatientHistoryAsDoctor } from "./patientResultHistoryApi";
...
describe("HTTP Client & API layer", () => {
  ...
  it("throws ApiError with message on non-ok responses", async () => {
    ...
    try {
      ...
      expect((e as ApiError).message).toBe("Specific API Error Message");
    }
  });
  ...
  it("executes getPatientHistoryAsDoctor with referral query params", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: async () => ({ patientId: "Patient-01", entries: [] }),
    ...
    const res = await getPatientHistoryAsDoctor("Patient-01", "tenant-a", "Doctor-01");
    expect(res).toEqual({ patientId: "Patient-01", entries: [] });
    const [calledUrl] = mockFetch.mock.calls[0];
    expect(calledUrl).toContain("/api/results/history/patient/Patient-01?");
    expect(calledUrl).toContain("callerRoleCode=REFERRING_DOCTOR");
    expect(calledUrl).toContain("callerId=Doctor-01");
  });

  it("propagates a 403 ApiError from getPatientHistoryAsDoctor when the doctor has not referred the patient", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: false,
      status: 403,
      statusText: "Forbidden",
      json: async () => ({ message: "DELIVERY_DOCTOR_REFERRAL_MISMATCH" }),
    });

    await expect(
      getPatientHistoryAsDoctor("Patient-99", "tenant-a", "Doctor-01"),
