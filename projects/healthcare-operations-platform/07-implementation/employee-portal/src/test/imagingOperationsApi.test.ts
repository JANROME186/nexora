...
describe("imagingOperationsApi", () => {
  ...
  it("schedules an appointment slot via POST", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/v1/imaging/appointments", expect.objectContaining({ method: "POST" }));
  });
  ...
  it("fetches single appointment slot", async () => {
    ...
    expect(res).toEqual({ id: "SLOT-1" });
    expect(fetchMock).toHaveBeenCalledWith("/api/v1/imaging/appointments/SLOT-1", expect.objectContaining({ method: "GET" }));
  });
  ...
  it("lists appointment slots for patient", async () => {
    ...
    expect(res).toEqual([{ id: "SLOT-1" }]);
    expect(fetchMock).toHaveBeenCalledWith("/api/v1/imaging/appointments?patientId=PAT-1", expect.objectContaining({ method: "GET" }));
  });
  ...
  it("updates appointment slot status via PUT", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/v1/imaging/appointments/SLOT-1/status", expect.objectContaining({ method: "PUT" }));
  });
  ...
  it("registers reception check-in via POST", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/v1/imaging/receptions", expect.objectContaining({ method: "POST" }));
  });
  ...
  it("creates, gets, lists and updates studies", async () => {
    ...
    expect(await createStudy({ accessionNumber: "ACC-1", patientId: "PAT-1", modality: "CT" })).toEqual({ id: "STUDY-1" });
    ...
    expect(await updateStudyStatus("STUDY-1", { seriesCount: 2, instanceCount: 100, status: "COMPLETED" })).toEqual({ id: "STUDY-1", status: "COMPLETED" });
  });

  // BCM-IMG-004: DICOM Integration
  it("handles DICOM configs and operations", async () => {
    mockFetchOnce({ jsonBody: { id: "CFG-1" } });
    expect(await registerDicomConfig({ aeTitle: "PACS", host: "127.0.0.1", port: 104, modalityType: "CT" })).toEqual({ id: "CFG-1" });
    ...
    expect(await validateDicomHeader("CFG-1", "PAT-1", "UID-1", "CT")).toEqual({ validHeader: true });
  });

  // BCM-IMG-005: PACS Integration
  it("handles PACS endpoints and web operations", async () => {
    mockFetchOnce({ jsonBody: { id: "PACS-1" } });
    expect(await registerPacsEndpoint({ pacsNodeId: "NODE-1", baseUrl: "https://pacs.org", protocol: "DICOM_WEB" })).toEqual({ id: "PACS-1" });
    ...
    expect(await qidoSearchPacsStudies("PACS-1", "PAT-1", "CT")).toEqual([{ studyInstanceUid: "UID-1" }]);
    ...
    expect(await stowStorePacs("PACS-1", "UID-1", "application/dicom", "base64")).toEqual({ status: "STORED" });
  });

  // BCM-IMG-006: Dictation
  it("handles medical dictations", async () => {
    mockFetchOnce({ jsonBody: { id: "DICT-1" } });
    expect(await createDictation({ studyId: "STUDY-1", dictationText: "Text" })).toEqual({ id: "DICT-1" });
  ...
  it("handles radiology reports and signature", async () => {
    ...
    expect(await createRadiologyReport({ studyId: "STUDY-1", findingsText: "Fine", impressionText: "Normal" })).toEqual({ id: "REP-1" });
  ...
  it("handles study delivery packages", async () => {
    ...
    expect(await createDeliveryPackage({ studyId: "STUDY-1", patientId: "PAT-1", deliveryFormat: "PDF" })).toEqual({ id: "PKG-1" });
    ...
    expect(await markDeliveryPackageDelivered("PKG-1")).toEqual({ id: "PKG-1", status: "DELIVERED" });
  });
});
