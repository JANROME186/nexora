...
artifact:
  ...
  machine_readable: open-data-ingestion-contract.md
  owner: Nexora Product Architecture Team
  implements_framework_standard: ../../../../nexora-framework/02-standards/standards/open-data-ingestion-standard.md
purpose: Define simple, provider-deliverable ingestion formats and validation processes
  for migrating a diagnostic laboratory from another platform into HOP.
framework_standard:
  id: NXF-ODI-STD-001
  name: Open Data Ingestion Standard
  path: ../../../../nexora-framework/02-standards/standards/open-data-ingestion-standard.md
capability:
  id: BCM-PLT-010
...
principles:
- Migration must never depend on proprietary vendor cooperation beyond exporting ordinary
  business data.
- Any incumbent provider should be able to deliver the required information using
  simple open formats.
...
- file: manifest.yaml
  required: true
  purpose: Declares source system, export date, format, entity files, row counts,
    checksums and contact owner.
- file: patients.*
...
- step: verify_manifest
  description: Validate declared files, row counts, encoding, checksums and required
    metadata.
- step: profile_data
  description: Detect columns, data types, null rates, duplicates and reference consistency.
- step: map_to_canonical_model
  description: Map source fields to HOP canonical entities through an anti-corruption
    mapping.
- step: dry_run_validation
  description: Validate without mutation and produce row-level errors, warnings and
...
- step: reconcile
  description: Produce imported, rejected, skipped and warning counts by entity and
    file.
...
provider_request_policy:
  minimum_request: Export the in-scope data in CSV, XLSX, JSON or NDJSON with a manifest
