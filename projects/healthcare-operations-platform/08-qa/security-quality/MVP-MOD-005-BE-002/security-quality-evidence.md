...
```yaml
artifact:
  id: HOP-SQ-MVP-MOD-005-BE-002
  type: security-quality-evidence
  name: MVP-MOD-005-BE-002 Security and Quality Evidence
  version: 1.0.0
  status: passed
...
checks:
  ...
  cyclonedx_sbom:
    ...
    - 07-implementation/backend/target/classes/META-INF/sbom/application.cdx.json
  dependency_vulnerability_scan_all_severities:
    status: passed
    command: mvn -gs .mvn/global-settings.xml --settings .mvn/settings.xml -Pquality
      org.owasp:dependency-check-maven:check
    result: BUILD SUCCESS; 0 dependency vulnerabilities reported by the configured
      gate
  trivy_security_scan_all_severities:
    status: passed
    command: trivy fs --scanners vuln,secret,misconfig --severity UNKNOWN,LOW,MEDIUM,HIGH,CRITICAL
      --exit-code 1 --skip-dirs .m2 --skip-dirs target --timeout 10m --format json
      --output target/trivy-backend-security-report.json .
    result: 0 vulnerabilities, 0 secrets and 0 misconfigurations
  trivy_license_review:
    status: reviewed_with_no_blocking_security_finding
    command: trivy fs --scanners vuln,secret,misconfig,license --severity UNKNOWN,LOW,MEDIUM,HIGH,CRITICAL
      --exit-code 1 --skip-dirs .m2 --skip-dirs target --timeout 10m --format json
      --output target/trivy-backend-report.json .
    result: License classifications reported for open source dependencies; no vulnerability,
      secret or misconfiguration findings.
    disposition: License review remains under backend release readiness policy and
      does not block this backend feature closure.
  coverage:
    status: passed_no_regression_and_improved
  ...
  secrets_scan:
    status: passed
    command: Trivy secret scan plus repository token pattern scan before closure
  agent_agnostic_scan:
    status: passed
    command: repository scan for named-agent or vendor-runtime requirements in touched
      implementation and documentation artifacts
  message_externalization_i18n_review:
    status: passed_with_existing_debt_tracking
    result: New backend errors use normalized error codes and existing exception paths;
      full localization adoption remains tracked by TD-I18N-002.
  dast_for_runnable_web_or_api_surfaces:
    status: deferred_to_MVP_MOD_005_QA_001_integrated_runtime_validation
    reason: BE-002 changes backend flows and executes local database validation; integrated
      API DAST remains expected in module QA after frontend surface is compiled.
    blocking: false
...
open_source_first:
  ...
  notes:
  - Backend continues to use the Java/Maven/Spring open source stack and configured
    quality toolchain.
  - The fiscal adapter boundary is provider-agnostic and replaceable by country-pack
    adapters.
decision:
