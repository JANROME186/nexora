...
artifact:
  ...
  name: MVP-MOD-003-BE-001 People and Clinical Master Data Backend Security Quality
    Evidence
  version: 1.0.0
  status: passed
...
scope:
  ...
  objective: Apply open-source-first security and quality gates to the backend outputs
    compiled for the People and Clinical Master Data capability packages.
open_source_first:
  status: passed
...
- id: SQ-003
  ...
  working_directory: 07-implementation/backend
  result: passed
  notes: peopleclinicalmasterdata module declares only organizationmanagement and
    auditcompliance as allowed dependencies. No boundary violations reported by ApplicationModules
    verification.
- id: SQ-004
  ...
  working_directory: 07-implementation/backend
  result: passed
  notes: Every operation declared in the four capability packages' openapi-source.md
...
- id: SQ-007
  ...
  command: trivy fs --scanners vuln,secret,misconfig --severity HIGH,CRITICAL --exit-code
    1 --no-progress --skip-dirs "backend/.m2,backend/target,employee-portal/node_modules,
  ...
  notes: 0 vulnerabilities, 0 secrets and 0 misconfigurations detected. Backend pom.xml
    scanned as pom target and employee-portal/package-lock.json scanned as npm target.
- id: SQ-008
  name: Agent-agnostic scan
  command: PowerShell Select-String scan of created Java, YAML and SQL artifacts for
    named-agent, assistant, vendor or cloud-runtime requirements.
  working_directory: 07-implementation/backend
  result: passed
...
- id: SQ-009
  ...
  notes: OWASP ZAP is not part of the local toolchain yet. Manual MockMvc coverage
    exercises every endpoint. TD-QA-001 tracks automated DAST enablement.
- id: SQ-010
  name: Container or IaC scan
  command: trivy fs (same command as SQ-007)
  result: passed
  notes: Compose and runtime files are covered by the same filesystem misconfiguration
    scan.
dependency_remediation:
  changes_applied: false
  notes: No new dependencies were added. Runtime remains on Spring Boot 3.5.14 and
    PostgreSQL JDBC 42.7.12 as validated in the MVP-MOD-002 CLOSEOUT security evidence.
technical_debt:
  registered_reused:
  - TD-QA-001
  - TD-BE-002
...
readiness:
  ...
  next_required_focus:
  - Implement the deferred custom rules for BCM-PER-001/002/003 and BCM-ATT-002 identified
    in 08-qa/qa/people-and-clinical-master-data/MVP-MOD-003-BE-001-validation.md.
