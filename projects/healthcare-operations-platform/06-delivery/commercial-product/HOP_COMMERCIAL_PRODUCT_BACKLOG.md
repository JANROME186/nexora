...
HOP has advanced through `MVP-MOD-005-BE-002` and `HOP-QUALITY-ALIGNMENT`
(`HOP-QA-ALIGN-001` through `HOP-QA-ALIGN-CLOSEOUT`) is closed. Functional development
continues with `MVP-MOD-005-FE-001`.
...
`MVP-MOD-005-BE-002` is closed. Continue with `MVP-MOD-005-FE-001` from:
