...
Current active backlog item: `COM-MOD-013-FE-001`.

Latest update: `COM-MOD-013-BE-001` is closed. Compiled backend outputs for External Quality Control (`BCM-QLT-002`), CAPA Management (`BCM-QLT-006`), Audit Management (`BCM-QLT-007`), Audit Trail Export (`BCM-PLT-007`), Document Management (`BCM-PLT-008`), and Quality Event Intake in new `externalqualitycompliance` Spring Modulith module. Resolved `TD-BE-016` by implementing filtered audit event search and `POST /api/audit/events/export` generating downloadable compliance packages. Added 4 new `PermissionCode` constants (`SCREEN_EXTERNAL_QUALITY_CONTROLS`, `SCREEN_CAPA_MANAGEMENT`, `SCREEN_AUDIT_MANAGEMENT`, `SCREEN_DOCUMENT_MANAGEMENT`), localized error catalogs in es-MX/en-US, and created `db/external-quality-and-compliance/schema.sql`. 381 unit and integration tests passing clean; backend line coverage floor preserved above 84.14%. Next active backlog item: `COM-MOD-013-FE-001`.
