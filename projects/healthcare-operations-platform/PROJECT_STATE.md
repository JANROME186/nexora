---
artifact:
  id: PROJECT_STATE
  type: project-state-index
  status: active
  optimization: atomic_context
---

# Project State

<!-- NEXORA_STRUCTURED_PAYLOAD_V1 -->

## Structured Payload

```yaml
project: Healthcare Operations Platform
project_slug: healthcare-operations-platform
company: Nexora
repository_version: 1.0.0
current_phase: HOP-HARD-FE-001 closed. Final hardening continues with mobile, patient portal, doctor portal and imaging delivery hardening.
commercial_product_progress:
  status: active
  current_iteration: HOP-HARD-APP-001
  active_module: HOP-FINAL-HARDENING
  active_backlog_item: HOP-HARD-APP-001
  execution_prompts: 06-delivery/commercial-product/HOP_COMMERCIAL_BACKLOG_EXECUTION_PROMPTS.md
  backlog: 06-delivery/commercial-product/HOP_COMMERCIAL_PRODUCT_BACKLOG.md
  capability_package_index: 01-product-definition/business-capabilities/packages/capability-package-index.md
  detail_ref: 08-qa/project-tracking/progress-ledger/commercial-product-progress-detail.md
architecture_status: Architecture Freeze v1.0
implementation_progress:
  current_backlog_item: HOP-HARD-APP-001
  active_backlog_item: HOP-HARD-APP-001
  next_backlog_item: HOP-HARD-WEB-001
  current_module: HOP-FINAL-HARDENING
  progress_percent: null
  last_commit: null
  completed_backlog_items_count: 112
  recent_completed_backlog_items:
  - HOP-HARD-FE-001
  - HOP-HARD-DATA-001
  - HOP-HARD-IAM-001
  - HOP-HARD-BE-001
  - COM-MOD-015-CLOSEOUT
  - COM-MOD-015-QA-001
  - COM-MOD-015-BE-002
  - COM-MOD-015-BE-001
  - COM-MOD-015-DEF
  - COM-MOD-014-BE-001
  - COM-MOD-014-INT-001
  - COM-MOD-014-FE-001
module_closeout:
  active_ref: 08-qa/project-tracking/progress-ledger/module-closeout-detail.md
  load_on_demand: true
recent_completed_deliverables_index:
- id: COM-MOD-015-CLOSEOUT
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-015-QA-001
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-015-BE-002
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-014-CLOSEOUT
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-015-DEF
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-014-QA-001
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-014-FE-001
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-014-INT-001
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-014-BE-001
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-014-DEF
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-017-CLOSEOUT
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-017-QA-001
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-017-FE-001
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-017-BE-002
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: NXF-FMT-001
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
- id: COM-MOD-017-BE-001
  ledger_ref: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
atomic_references:
  completed_deliverables_ledger: 08-qa/project-tracking/progress-ledger/completed-deliverables-ledger.md
  commercial_product_progress_detail: 08-qa/project-tracking/progress-ledger/commercial-product-progress-detail.md
  module_closeout_detail: 08-qa/project-tracking/progress-ledger/module-closeout-detail.md
  commercial_backlog_master_plan: 06-delivery/commercial-product/backlog-map/MASTER_BACKLOG_PLAN.md
  source_registry_index: 08-qa/project-tracking/source-registry/SOURCE_REGISTRY_INDEX.md
context_policy:
  load_this_file_first: true
  load_atomic_references_only_on_demand: true
  do_not_expand_completed_deliverables_by_default: true
```
